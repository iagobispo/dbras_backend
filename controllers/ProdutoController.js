...
class ProdutoController {
    ...
    // GET /disponiveis - indexDisponiveisLojasId
    async indexDisponiveisLojasId(req,res,next){
        const offset = Number(req.query.offset) || 0;
        const limit = Number(req.query.limit) || 30;
        try {
            const produtos = await Produto.paginate(
    ...
    // GET / - indexLojaId buscar com id da loja 
    async indexLojaId(req,res,next){
        const offset = Number(req.query.offset) || 0;
        const limit = Number(req.query.limit) || 30;
        try {
            const produtos = await Produto.paginate(
    ...
    async searchidloja(req,res,next){
        const offset = Number(req.query.offset) || 0;
        const limit = Number(req.query.limit) || 30;
